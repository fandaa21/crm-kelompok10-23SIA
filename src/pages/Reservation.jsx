...
const statusMap = {
  ...
  pending: { color: "bg-yellow-100 text-yellow-700", icon: <CalendarCheck className="w-4 h-4" />, label: "Menunggu" },
...
export default function Reservation() {
  ...
  // Load dari localStorage saat komponen pertama kali dirender
  useEffect(() => {
  ...
  return (
    <div className="p-6 space-y-6 font-sans bg-white min-h-screen">
      <div className="flex items-center gap-3 text-3xl font-bold text-blue-800">
      ...
      <div className="grid md:grid-cols-3 sm:grid-cols-2 gap-6">
        {reservations.map((res) => (
          <div key={res.id} className="rounded-xl border shadow-sm bg-gray-50 p-4 space-y-3">
      ...
      {showModal && (
        <div className="fixed inset-0 backdrop-blur-sm bg-white/30 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-xl w-full max-w-md shadow-lg space-y-4">
            <h2 className="text-xl font-bold">Form Tambah Reservasi</h2>
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={handleAddReservation} className="px-3 py-1 bg-blue-600 text-white rounded">Simpan</button>
            </div>
          </div>
        </div>
      )}
